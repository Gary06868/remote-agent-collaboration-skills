...
- `AGENTS.md` records shared rules, startup checks, Git rules, logging rules, and when to ask.
- `COLLAB_LOG.md` records Active Work Locks, blockers, decisions, updates, and handoffs.
...
| TP-004 | Soft locks | Verify lock fields are consistent across Lead, Member, and template files. | Shared field contract tests. |
| TP-005 | Template completeness | Verify templates contain the operational sections needed by agents. | Template heading and content checks. |
| TP-006 | Example flow | Verify the tiny-team example matches the advertised Lead/Member lifecycle. | Example consistency tests. |
| TP-007 | Link integrity | Verify local Markdown links in top-level docs and example docs resolve. | Local link tests. |
...
- `COLLAB_LOG.md` ordering is consistent: Active Work Locks appear before Current Summary because conflict avoidance depends on early lock visibility.
...
- Notes: Initial run exposed 2 overly strict assertions in the new test file. The assertions were corrected to validate the Skill contract instead of matching README-only phrasing, then the suite passed.
