...
4. 如果 Task Assignment Mode 已启用且存在 `TEAM_TASKS.md`，读取自己的任务。
...
如果 `TEAM_TASKS.md` 存在，但 `AGENTS.md` 写明 Task Assignment Mode 未启用，把它当作旧文件或参考材料。使用或更新前先询问用户。
...
默认完成规则：Member 完成分配工作后，标记为 `READY_FOR_REVIEW`，除非 `AGENTS.md`、Lead 或用户明确说明可以直接标记 `DONE`。
...
`COLLAB_LOG.md` 必须把 Active Work Locks 区块放在靠前位置。
...
任何较大的读写任务开始前，Agent 都应该检查是否有重叠 lock。以下情况通常视为重叠：

- 同一个文件路径。
- 同一个文件夹或模块。
- 较大的模块 lock 覆盖了你的文件路径。
- 两个任务都可能修改同一个跨模块接口。

如果没有冲突，添加自己的 lock，写明 actor、role、scope 和预计工作。如果有冲突，不要编辑。告诉用户哪个 actor 正在处理重叠范围，并询问下一步。

如果 lock 看起来过期，可以标记或报告 stale，但不要在没有用户确认时移除其他 actor 的 lock。建议 stale 时间：2 小时。
